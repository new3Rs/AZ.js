...
Board.prototype.setRotate180 = function(value) {
...
var Canvas = function(elem, opt, images) {
  ...
  this.listeners = {'click': [], 'mousemove': [], 'mouseout': [], 'mousedown': []};
  ...
  if (typeof window.ontouchstart === 'undefined') {
    ...
    canvas.ontouchend = function(ev) {
      var c = this.getCoordinate(ev.changedTouches[0].clientX, ev.changedTouches[0].clientY),
          listeners = this.listeners.click;
  ...
  if (typeof window.ontouchstart === 'undefined') {
    canvas.onmousemove = function(ev) {
      ...
      canvas.ontouchmove = function(ev) {
        ...
        var c = this.getCoordinate(ev.touches[0].clientX, ev.touches[0].clientY),
            listeners = this.listeners.mousemove;
  ...
  canvas.onmouseout = function(ev) {
    ...
    lastMove = new Coordinate(-1,-1);
    for(var l=0; l<listeners.length; l++)
      listeners[l].call(this, ev);
  }.bind(this);

  // Mousedown handler will again call all listeners of that event, no
  // coordinates will be passed of course, only the event
  if (typeof window.ontouchstart === 'undefined') {
      canvas.onmousedown = function(ev) {
        var c = this.getCoordinate(ev.clientX, ev.clientY);
        if (c.i < 0 || c.i >= this.opt.board.width || c.j < 0 || c.j >= this.opt.board.height) {
          return
        }
        var listeners = this.listeners.mousedown;
...
Canvas.prototype.restore = function(x, y, w, h) {
  ...
  try {
    this.ctx.clearRect(x, y, w, h); // to support alpha < 1
 ...
 */
Canvas.prototype.draw = function(jboard, i1, j1, i2, j2, rotate180) {
  ...
  jboard.each(function(c, type, mark) {
    ...
    if(mark) this.stones.drawMark(this.ctx, mark, ox, oy, rotate180);
  }.bind(this), i1, j1, i2, j2); // provide iteration limits
...
/**
 * remove nth child node.
 *
 * @param {Integer} n index of removed element
 */
Node.prototype.remove = function(n) {
  var deleted = this.children.splice(n, 1);
  if (deleted.length === 0)
    return
    ...
    setTimeout(function() { // schedule update in the end
      for(var c=0; c<this.canvases.length; c++)
        this.canvases[c].draw(this.board, this.min.i, this.min.j,
          this.max.i, this.max.j, this.board.rotate180);
...
Notifier.prototype.changeBoard = function(board) {
  ...
  for(var c=0; c<this.canvases.length; c++)
    this.canvases[c].draw(this.board, 0, 0, this.board.width, this.board.height, this.board.rotate180);
};
...
Setup.prototype.create = function(elemId, readyFn) {
  ...
  var createCallback = function(images) {
    ...
    jcanvas.draw(this.board, 0, 0, this.board.width-1, this.board.height-1, this.board.rotate180);
...
function sgfMove(node, name, values, moveMarks, gameType) {
  ...
  if (gameType === '4') {
      if (node.jboard.getType(coord) === C.CLEAR) {
          play = { success: true, captures: [], ko: false };
      } else {
          play = { success: false, captures: [], ko: false };
      }
  } else {
...
function recurseRecord(jrecord, gameTree, moveMarks) {
  for(var i=0; i<gameTree.sequence.length; i++) {
    ...
    var gameType = jrecord.getRootNode().info.gameType;
    ...
    for(var key in node) {
      if(node.hasOwnProperty(key)) {
        ...
        if(!SGFProperties[key](jnode, key, node[key], moveMarks, gameType)) {
...
Stones.prototype.drawMark = function(ctx, mark, ox, oy, rotate180) {
  switch(mark) {
    case C.MARK.SQUARE:
      ctx.beginPath();

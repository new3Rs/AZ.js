...
class TFProcess:
    def __init__(self):
        ...
        self.VALUE_FULLY_CONNECTED = 256

        # For exporting
        self.weights = []

        self.logbase = 'leelalogs'
    ...
    def init(self, batch_size, macrobatch=1, logbase='leelalogs'):
        ...
        if self.DATA_FORMAT == 'NCHW':
            planes = tf.reshape(planes, (batch_size, self.FEATURES, self.BOARD_SIZE*self.BOARD_SIZE))
            probs = tf.reshape(probs, (batch_size, self.BOARD_SIZE*self.BOARD_SIZE + 1))
            winner = tf.reshape(winner, (batch_size, 1))
        else:
            planes = tf.reshape(planes, (batch_size, self.BOARD_SIZE*self.BOARD_SIZE, self.FEATURES))
            probs = tf.reshape(probs, (batch_size, self.BOARD_SIZE*self.BOARD_SIZE + 1))
            winner = tf.reshape(winner, (batch_size, 1))
    ...
    def residual_block(self, inputs, channels, name=None):
        ...
        net = conv2d(net, W_conv_1, self.DATA_FORMAT)
        net = self.batch_norm(net)
        net = tf.nn.relu(net)
    ...
    def construct_net(self, planes):
        ...
        h_fc1 = tf.add(tf.matmul(h_conv_pol_flat, W_fc1), b_fc1, name="fc1")
        ...
        W_fc2 = weight_variable([self.BOARD_SIZE * self.BOARD_SIZE, self.VALUE_FULLY_CONNECTED])
        b_fc2 = bias_variable([self.VALUE_FULLY_CONNECTED])
        self.weights.append(W_fc2)
        self.weights.append(b_fc2)
        h_fc2 = tf.nn.relu(tf.add(tf.matmul(h_conv_val_flat, W_fc2), b_fc2))
        W_fc3 = weight_variable([self.VALUE_FULLY_CONNECTED, 1])
        b_fc3 = bias_variable([1])
        self.weights.append(W_fc3)
        self.weights.append(b_fc3)
        h_fc3 = tf.nn.tanh(tf.add(tf.matmul(h_fc2, W_fc3), b_fc3), name="fc3")
